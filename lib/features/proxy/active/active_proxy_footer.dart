...
import 'package:reaeeman/features/proxy/active/ip_widget.dart';
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _InfoProp(
                        ...
                        text: SpUtil.getInt('expired') == 0
                            ? t.profile.subscription.status.permanent
                            : (SpUtil.getInt('expired') ?? 0) > 0
                                ? t.profile.subscription.status.remainingDays(
                                    days: _calculateRemainingDays(
                                        SpUtil.getInt('expired') ?? 0))
                                : t.profile.subscription.status.expired,
                        semanticLabel: SpUtil.getInt('expired') == 0
                            ? t.profile.subscription.status.permanent
                            : (SpUtil.getInt('expired') ?? 0) > 0
                                ? t.profile.subscription.status.remainingDays(
                                    days: _calculateRemainingDays(
